...
var cf = {
  ...
  {
    // update data only if 12 hours has passed and user visits a checkout page
    if (cf.dataUpdateNeeded) {
      cf.checkUpdates();

      // next update after 12 hours
      setTimeout(function () {
        cf.dataUpdateNeeded = true;
      }, 1000 * 60 * 60 * 12);
    }
  },
...
function requestHandler(request, sender, sendResponse) {
  ...
  else if (request.call == "checkForUpdatesIfNeeded") {
    cf.checkForUpdatesIfNeeded();
  }
}
