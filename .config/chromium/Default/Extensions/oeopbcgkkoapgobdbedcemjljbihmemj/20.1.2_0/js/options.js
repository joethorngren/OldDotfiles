	...
	polymerPromise.then(function() {
		...
		$("#dndSchedule").click(function() {
			...
			for (var a=1; a<8; a++) {
				...
				$allDay.attr("day", a%7);
				$allDay.off().click(function () {
					if (donationClicked("DND_schedule")) {
			...
			$dialog.find(".okDialog").off().click(function () {
				...
				Settings.store("DND_schedule", atleastOneChecked);
				// store actual hours
		...
		if (Settings.read("accountAddingMethod") == "autoDetect" && $("#showNotificationEmailImagePreview").length) {
